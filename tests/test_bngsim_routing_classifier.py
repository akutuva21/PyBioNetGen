...
class TestBngsimRouteClassifier:
    ...
    @pytest.mark.parametrize(
        ...
        [
            [
                _action("setParameter", {'"k"': None, "2": None}),
                _action("simulate_ode"),
            ],
            [
                _action("setConcentration", {'"A()"': None, "10": None}),
                _action("simulate_ode"),
            ],
...
@pytest.mark.parametrize("action_text", COMPLEX_BNGL_ACTION_CASES)
def test_library_complex_bngl_uses_bngsim_route_not_subprocess_classifier(
    tmp_path, action_text
):
...
def test_run_bngl_with_bngsim_complex_action_uses_backend_hook_without_executor(
    tmp_path,
):
...
def test_run_bngl_with_bngsim_protocol_uses_backend_hook_without_python_parser(
    tmp_path,
):
    from bionetgen.core.tools.bngsim_bridge import run_bngl_with_bngsim
...
def test_run_bngl_with_bngsim_scan_uses_backend_hook_without_python_scan_outputs(
    tmp_path,
):
    from bionetgen.core.tools.bngsim_bridge import run_bngl_with_bngsim
