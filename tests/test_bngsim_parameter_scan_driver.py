...
def test_scan_values_backward_pass_is_reversed_forward():
    ...
    fwd = scan_values(
        _make_request(par_min=1.0, par_max=1e2, n_scan_pts=10, log_scale=True)
    )
    bwd = scan_values(
        _make_request(par_min=1e2, par_max=1.0, n_scan_pts=10, log_scale=True)
    )
...
def test_detect_parses_print_functions():
    ...
    assert (
        detect_inprocess_scan(_valid_sequence(print_functions="1")).print_functions
        is True
    )
    assert (
        detect_inprocess_scan(_valid_sequence(print_functions="0")).print_functions
        is False
    )
...
def test_write_scan_file_matches_bng2_format(tmp_path):
    ...
    assert (
        lines[0]
        == "# " + f"{'k1':>14}" + " " + f"{'obs_a':>16}" + " " + f"{'obs_b':>16}"
    )
    # data rows: %16.8e fields, single-space separated
    assert (
        lines[1]
        == f"{1.0e-3:16.8e}" + " " + f"{1.234e3:16.8e}" + " " + f"{5.0e1:16.8e}"
    )
    assert len(lines) == 3
...
def test_write_bifurcation_file_matches_bng2_format(tmp_path):
    ...
    assert (
        lines[0]
        == "# " + f"{'Kxy':>14}" + " " + f"{'A_fwd':>16}" + " " + f"{'A_bwd':>16}"
    )
...
def test_ssa_scan_fast_path_is_taken_and_reproducible(tmp_path, caplog):
    ...
    assert any(
        "fast path" in rec.message and "ssa" in rec.message for rec in caplog.records
    )
