...
class TestWriteBngsimResults:
    ...
    def test_nfsim_fallback_skipped_when_bngsim_returns_expressions(self):
        ...
        result = _make_mock_result(
            obs_names=["obsA"],
            obs_data=np.zeros((3, 1)),
            n_times=3,
            func_names=["actually_from_bngsim"],
            func_data=func_data,
        )
...
class TestRunNfsim:
    ...
    def test_gml_is_set(self):
        ...
        with (
            ...
            mock_bngsim.NfsimSession.assert_called_once_with(
                xml_path, molecule_limit=100000
            )
...
class TestPartitionSimulatorOptions:
    ...
    def test_steady_state_tol_forwarded_as_float(self):
        _, run_kwargs = self._partition(
            {"steady_state": "1", "steady_state_tol": "1e-9"}
        )
