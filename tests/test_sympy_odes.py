...
def test_export_sympy_odes_exception():
    ...
    with patch("bionetgen.modelapi.sympy_odes.extract_odes_from_mexfile") as mock_extract:
        ...
        with pytest.raises(BNGError, match="Failed to extract ODEs from mex C file: dummy_path"):
            mock_model = MagicMock(spec=bngmodel)
            with patch("bionetgen.modelapi.sympy_odes._find_mex_c_file", return_value="dummy_path"):
