...
def _sniff_xml_format(file_path):
    ...
    try:
        ...
        raise BNGFormatError(
            file_path, f"Could not read file for format detection: {e}"
        ) from e
...
def _write_nf_final_state_species(session, output_dir, output_root):
    ...
    try:
        ...
        logger.warning(
            "get_final_state: save_species failed (%s); final-state writeback skipped.",
            exc,
        )
...
def _collapse_nfsim_concentration_changes(
    ...
    if conc_overrides:
        for species_pattern, target_count in conc_overrides.items():
            ...
            try:
                collapsed_overrides[mol_type] = collapsed_overrides.get(
                    mol_type, 0
                ) + int(target_count)
    ...
    if conc_deltas:
        for species_pattern, delta_count in conc_deltas.items():
            mol_type = str(species_pattern).split("(", 1)[0]
            try:
                collapsed_deltas[mol_type] = collapsed_deltas.get(mol_type, 0) + int(
                    delta_count
                )
            except Exception as e:
                logger.warning(
...
def _apply_nfsim_concentration_changes(
    ...
    if (
        ...
        remaining_deltas = {
            str(species_pattern): delta
            for species_pattern, delta in (conc_deltas or {}).items()
        }
        ...
        if conc_overrides:
            for species_pattern, target_count in conc_overrides.items():
                try:
                    ...
                    desired_count = int(target_count) + int(
                        remaining_deltas.pop(pattern, 0)
                    )
...
def _partition_simulator_options(sim_options):
    ...
    if dropped:
        logger.debug(
            "Direct BNGsim job: ignoring non-Simulator options %s", sorted(dropped)
        )
            ...
            if param_overrides:
                for pname, pval in param_overrides.items():
                    try:
                        ...
                        logger.debug(
                            "NFsim: set_param(%s, %s) skipped: %s", pname, pval, exc
                        )
...
def _try_prepare_codegen(net_path):
    ...
    try:
        ...
        logger.warning(
            "Codegen compilation failed (%s); falling back to interpreted ODE RHS (slower)",
            e,
        )
        return ""
...
def run_bngl_with_bngsim(
    ...
    **sim_kwargs,
    ...
    if not BNGSIM_AVAILABLE:
        raise BNGSimError(
            f"BNGsim is not usable: {BNGSIM_UNAVAILABLE_REASON or 'unknown reason'}."
        )
