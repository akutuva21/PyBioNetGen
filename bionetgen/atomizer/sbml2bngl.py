        ...
        for arule in self.model.getListOfRules():
            ...
            # it is an assigment rule
                ...
                def _track_assignment_rule(
                    ...
                    if target_name in self.used_molecules:
                        raise TranslationException(
                            f"Variable {target_name} is used both in a reaction and as an assignment rule. "
                            "This is not supported in BioNetGen."
                        )
                ...
                if rawArule[0] in zRules:
                    ...
                    if not molecules[rawArule[0]]["isBoundary"]:
                        ...
                        # We used to log WARNING:ARUL004 here, but since it's a boundary condition
                        # it won't be changed by a reaction unless it's explicitly done so.
                        # Since we check self.used_molecules in _track_assignment_rule,
                        # it will throw an exception if there's a conflict. We don't need to log here.
                        self.only_assignment_dict[name] = name + "_ar"
                        self.bngModel.add_arule(arule_obj)
                        continue
                else:
