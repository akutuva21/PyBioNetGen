...
process_one() {
    ...
    if [ $MERGE_EXIT -ne 0 ]; then
        ...
        # Handle patch_sbml2bngl.py
        if git ls-files -u patch_sbml2bngl.py 2>/dev/null | grep -q .; then
            git checkout --theirs patch_sbml2bngl.py 2>/dev/null
            git add patch_sbml2bngl.py
        fi

        # Handle each specific known conflict file
        for f in bionetgen/modelapi/runner.py tests/test_bng_models.py tests/test_csimulator.py bionetgen/atomizer/bngModel.py; do
        ...
        # Handle remaining conflicts: for test files, accept theirs; for code files, accept theirs
        CONFLICTS=$(git diff --name-only --diff-filter=U 2>/dev/null)
        if [ -n "$CONFLICTS" ]; then
            echo "Still conflicting:" $CONFLICTS
